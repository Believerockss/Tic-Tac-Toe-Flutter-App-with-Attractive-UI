...
class FindGame {
  ...
  createGames(entryFee, round) {
    ...
    var create = CreateGame(player1: _auth.currentUser!.uid.toString(), entryfee: entryFee, round: round, tryy: player);
  ...
  Future<Map<String, dynamic>> joinGame(entryFee, round) async {
    ...
    if (dta.snapshot.value != null) {
      ...
      for (int i = 0; i < valMap.length; i++) {
        ...
        if (player1 != FirebaseAuth.instance.currentUser!.uid && status == "pending" && _entryFee == entryFee && _round == round) {
          gameAvailable = true;
      ...
      if (!gameAvailable && joinGameStatus != JoinStatus.created) {
        ...
        _ins.ref().child("Game").child(roomKey).child("player2").update({"id": _auth.currentUser!.uid, "won": 0});

        joinGameStatus = JoinStatus.joined;
      }
    }

    Map<String, dynamic> map = Map.from({"JoinStatus": joinGameStatus, "roomKey": roomKey, "oppornentKey": oppornentKey});
