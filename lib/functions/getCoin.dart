...
class GetUserInfo {
  ...
  getCoin() async {
    DatabaseEvent snap = await _db.ref().child("users").child(FirebaseAuth.instance.currentUser!.uid).once();
  ...
  setProfilePic(var value) async {
    await _db.ref().child("users").child(FirebaseAuth.instance.currentUser!.uid).update({"profilePic": value});
  }

  setUsername(var value) async {
    await _db.ref().child("users").child(FirebaseAuth.instance.currentUser!.uid).update({"username": value});
  }
  ...
  Future detectChange(String field, Function(dynamic value) callBack) async {
    dynamic _coin = 0;
    _db.ref().child("users").child(FirebaseAuth.instance.currentUser!.uid).onChildChanged.listen((DatabaseEvent ev) {
