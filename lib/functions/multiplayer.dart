...
class Multiplayer {
  ...
  static updateLocalList(String? gameKey, dynamic dbIns, void Function(dynamic b) update) {
    _stream = dbIns.ref().child("Game").child(gameKey).child("buttons").onChildChanged.listen((DatabaseEvent ev) {
      update(ev);
    });
  }
  ...
  static checkStatus(
    ...
    for (var j = 0; j < winningCondition.length; j++) {
      if (buttons[winningCondition[j][0]]["player"] == buttons[winningCondition[j][1]]["player"] &&
          buttons[winningCondition[j][1]]["player"] == buttons[winningCondition[j][2]]["player"] &&
